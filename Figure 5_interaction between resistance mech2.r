...
for(j in 1:5){
  for(i in 1:21){
    ...
    data6_res[,1,j] =  read.table(paste("F:/Ellies_output_folder/behav_phys_resistance6/draw_0/behav_phys_resistance6",sites_init[j]+res_val[1],"0.txt",sep="_"),header=TRUE)[,1]
    data6_res[,i+1,j] = read.table(paste("F:/Ellies_output_folder/behav_phys_resistance6/draw_0/behav_phys_resistance6",sites_init[j]+res_val[i],"0.txt",sep="_"),header=TRUE)[,3]
...
zf1 = Eff1peren_LLIN$eff
zf2 = Eff1peren_LLIN_IRS$eff
zf3 = Eff1seas_LLIN$eff
zf4 = Eff1seas_LLIN_IRS$eff
zf5 = Eff1peren_LLIN_IRSact$eff
zf6 = Eff1seas_LLIN_IRSact$eff
...
polygon(c(data2_res[,1,1],rev(data2_res[,1,1])),c(data2_res[,2,2],rev(data2_res[,2,5])),border=NA,col=transp("darkred",0.4))
polygon(c(data2_res[,1,1],rev(data2_res[,1,1])),c(data2_res[,2,3],rev(data2_res[,2,4])),border=NA,col=transp("darkred",0.4))
lines(data2_res[,2,1] ~  data2_res[,1,1],lty=2,col="darkred",lwd=2)#median
...
for(i in 1:2){
  ...
  polygon(c(resistance,rev(resistance)),
          c(Efficacy_peren_LLIN50IRS80[,veca[i]],rev(Efficacy_peren_LLIN50IRS80[,vecb[i]])),
          border=NA,col=transp("darkred",0.4))
  
}
lines(Efficacy_peren_LLIN50[,1] ~ resistance,lty=1,lwd=2,col="blue")
lines(Efficacy_peren_LLIN50IRS80[,1] ~ resistance,lty=2,lwd=2,col="darkred")
...
matrix_funs(surface.matrix1pc,min(surface.matrix4pc),max(surface.matrix3pc),
...
matrix_funs(surface.matrix2pc,min(surface.matrix4pc),max(surface.matrix3pc),
            upps=30,uni = 5,levs = 6,colschoice = blue2green,cols_conts="gold3")
...
matrix_funs(surface.mat1,min(surface.mat3),max(surface.mat4),
...
matrix_funs(surface.mat2,min(surface.mat3),max(surface.mat4),
            upps=1,uni = 0.1,levs = 11,colschoice = heat.colors,cols_conts="darkblue")
...
text(x = -20,y = 4.85,"Perennial setting",cex = 1)
